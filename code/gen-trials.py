...
Requirements:
    1. Python 3.7
    2. Needs numpy and random packages
Output:
    1. Two N x M .cvs files; N = # of trials + 1; # of variables
    for stable first condition and volatile first condition
"""
...
# header list to write in csv trial list file
...
with open("vf_trials.csv","a", newline='') as fh:
    fh.write(",".join(header)) # write header to csv
    fh.write('\n') # space to add trial information
    fh.write('\n'.join('%s, %s, %s, %s, %s, %s, %s, %s, %s, %s'\
            ...
            # creates a tuple of lenght = trial info, and loop through the
            # tuple writing each variable 1 by 1; at the end adds a spcace
            # to write a new line
...
with open("sf_trials.csv","w") as fh:
    ...
    fh.write('\n') # space to add trial information
    fh.write('\n'.join('%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,'\
            % x for x in zip(trial_index, block, condition_sf, left_pos, \
            right_pos, target_sf, expected_key_sf,\
